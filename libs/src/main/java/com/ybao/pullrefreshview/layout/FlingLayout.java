...
public class FlingLayout extends FrameLayout implements NestedScrollingChild, NestedScrollingParent {
    ...
    protected Pullable mIsPullable;
    ...
    protected float mTempX;
    protected float mTempY;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mMaxDistance = getMeasuredHeight() / mPullMax;
    }
    ...
    private boolean canPullUp() {
        if (mPullView != null) {
            return mCanPullUp && mIsPullable.isGetBottom();
        }
        return mCanPullUp;
    }
    ...
    private boolean canPullDown() {
        if (mPullView != null) {
            return mCanPullDown && mIsPullable.isGetTop();
        }
        return mCanPullDown;
    }
     ...
     * 设置下拉最大值
     *
     * @param pullMax 范围[4,10]
     */
    public void setPullMax(int pullMax) {
        if (mPullMax == pullMax) {
            return;
        }
        mPullMax = pullMax < 4 ? 4 : (pullMax > 10 ? 10 : pullMax);
        requestLayout();
    }
...
//            Log.i("flingLayout", "onScrollChange:" + stateType);
    ...
    public void addView(View child, int index, ViewGroup.LayoutParams params) {
        if (mPullView == null && (mIsPullable = CanPullUtil.getPullAble(child)) != null) {
            mPullView = child;
        }
        ...
        if (mPullView != null && !ViewCompat.isNestedScrollingEnabled(mPullView)) {
            ...
            switch (ev.getActionMasked()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    //意图分析，避免误操作
                    int dataX = (int) (mx - mTempX);
                    int dataY = (int) (my - mTempY);
                    mTempX = mx;
                    mTempY = my;
                    if (mIsScrolling || (Math.abs(dataY) > Math.abs(dataX))) {
                        mIsScrolling = true;
                        if (moveY == 0) {
                            //开始时 在0,0处
                            //判断是否可以滑动
                            ...
                            if ((moveY < 0 && moveY + dataY >= 0) || (moveY > 0 && moveY + dataY <= 0)) {
                                ...
                                //是否超过最大距离
                                if (mMaxDistance == 0 || Math.abs(moveY) < mMaxDistance) {
                                    int ps = 0;
                                    int hDataY = dataY / 2;
                                    if (mMaxDistance == 0) {
                                        ps = (int) (-hDataY * Math.abs(moveY) / (float) mMaxDistance) - hDataY;
                                    } else {
                                        ps = (int) (-hDataY * Math.abs(moveY) / (float) mMaxDistance) - hDataY;
                                    }
                                    moveBy(ps + dataY);
                                } else if (moveY > mMaxDistance) {
                                    moveTo(mMaxDistance);
                                } else if (moveY < -mMaxDistance) {
                                    moveTo(-mMaxDistance);
                                }
                ...
                case MotionEvent.ACTION_POINTER_UP:
                    ...
                    if (mPointerId == pointerIdLeave) {
                        ...
                        // 调整触摸位置，防止出现跳动
                        mTempY = ev.getY(reIndex);
                    }
            }
            return super.dispatchTouchEvent(ev) || mIsScrolling;
        } else {
